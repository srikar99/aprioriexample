 ...
 *   $ java Apriori.Apriori fileName support
 *   $ java Apriori.Apriori /tmp/data.dat 0.8
...
public class Apriori extends Observable {
    ...
    private void go() throws Exception {
        ...
        log("Found "+nbFrequentSets+ " frequents sets for support "+((1-minSup)*100)+"% (absolute "+Math.round(numTransactions*minSup)+")");
        log("Done");
    }
        ...
        for(int i=0; i<itemsets.size(); i++)
        {
            for(int j=i+1; j<itemsets.size(); j++)
            {
                ...
                if (ndifferent==1) {
                    ...
                    // we have to create the hash myself using a String :-(
                	// we use Arrays.toString to reuse equals and hashcode of String
                	Arrays.sort(newCand);
